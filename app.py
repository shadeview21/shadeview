...
# Define folder paths and configure Flask app immediately after app creation
UPLOAD_FOLDER = 'uploads'
REPORT_FOLDER = 'reports'
INSTANCE_FOLDER = 'instance' # For SQLite database

app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['REPORT_FOLDER'] = REPORT_FOLDER
app.config['INSTANCE_FOLDER'] = INSTANCE_FOLDER # Ensure this is set in app.config
app.config['DATABASE'] = os.path.join(INSTANCE_FOLDER, 'shadeview.sqlite') # Database path

# Ensure these directories exist (using the defined variables directly)
...
def get_db():
    ...
    The connection is stored in Flask's `g` object for the current request.
    """
    if 'db' not in g:
...
def init_db():
    """
    Initializes the SQLite database schema, creating the 'patients' table if it doesn't exist.
    """
    db = get_db()
    db.execute('''
        CREATE TABLE IF NOT EXISTS patients (
            ...
            record_date TEXT,
            created_at TEXT,
            UNIQUE (user_id, op_number)
        );
    ''')
    db.commit()
    print(f"Database initialized: 'patients' table ensured in {app.config['DATABASE']}.")

# ===============================================
# Firestore Simulation Setup (for Canvas environment)
# ===============================================
...
def load_logged_in_user():
    ...
    Handles initial authentication token from Canvas environment.
    """
    ...
    g.firestore_user_id = g.user_id # Use this for Firestore paths
...
# ===============================================
# VITA Shade LAB Reference Values
# ===============================================
...
def extract_lab_features(img_bgr, mask=None):
    ...
    L = np.mean(l_channel) / 2.55
    a = np.mean(a_channel) - 128
    b = np.mean(b_channel) - 128
...
def train_model():
    ...
    try:
        ...
        model_to_train = SVC(kernel='rbf', probability=True, random_state=42)
...
# =========================================================
# Placeholder AI Modules for Advanced Analysis
# =========================================================

def detect_face_features(image_np_array):
    """
    ENHANCED PLACEHOLDER: Simulates detailed face feature extraction.
    Provides nuanced skin tone (including undertones), lip color, and eye contrast.
    """
    print("DEBUG: Simulating detailed Face Detection and Feature Extraction with color analysis...")
    avg_l = np.random.uniform(45, 80)
    avg_a = np.random.uniform(5, 25)
    avg_b = np.random.uniform(10, 30)
...
def segment_and_analyze_teeth(image_np_array):
    ...
    avg_l = np.random.uniform(50, 85)
    avg_a = np.random.uniform(-5, 10)
    avg_b = np.random.uniform(5, 25)
    simulated_overall_shade = np.random.choice(["A1", "A2", "A3", "B1", "B2", "C1", "C2", "D2"])
...
def calculate_confidence(delta_e_value, device_profile="ideal"):
    ...
    """
    base_confidence = np.random.randint(50, 95)
    ...
    try:
        ...
        if shade_classifier_model is not None:
            try:
                ...
                traceback.print_exc()
                overall_ml_shade = "ML Prediction Failed"
        else:
...
def dashboard():
    ...
    # conn.close() # get_db() is managed by teardown_appcontext
...
def save_patient_data():
    ...
    try:
        ...
        return redirect(url_for('dashboard'))
    # finally: # get_db() is managed by teardown_appcontext
    #     conn.close()
...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            # Save report metadata to Firestore (simulated)
            report_data = {
                'patient_name': patient_name,
                'op_number': op_number_from_form,
                'original_image': unique_filename,
                'report_filename': report_filename,
                'detected_shades': detected_shades, # This will be a dictionary
...
@login_required
def view_reports():
    """Renders the reports page, fetching reports from Firestore (simulated)."""
    try:
        reports_collection_path = ['artifacts', app_id, 'users', g.firestore_user_id, 'reports']
        user_reports_data = get_firestore_documents_in_collection(reports_collection_path)

        reports_for_template = []
        for report_data in user_reports_data:
            # The 'detected_shades' field is already a dictionary, no need to parse JSON
            shades = report_data.get('detected_shades', {})
            reports_for_template.append({
                'patient_name': report_data.get('patient_name', 'N/A'),
                'op_number': report_data.get('op_number', 'N/A'),
        ...
        return render_template('reports.html', reports=reports_for_template)
    except Exception as e:
        flash(f'Error retrieving reports: {e}', 'danger')
        traceback.print_exc()
...
if __name__ == '__main__':
    with app.app_context():
        # Ensure the instance directory exists before trying to remove the database file
        # This line is now safe because app.config['INSTANCE_FOLDER'] is set above.
        os.makedirs(app.config['INSTANCE_FOLDER'], exist_ok=True)
        db_path = app.config['DATABASE']
        if os.path.exists(db_path):
            print(f"INFO: Removing existing database file: {db_path}")
            os.remove(db_path)
        init_db() # Initialize the SQLite database when the app starts
    if shade_classifier_model is None:
